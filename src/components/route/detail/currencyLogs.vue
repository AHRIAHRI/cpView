	...
	export default {
		...
		methods:{
			...
			getData(currentPage,pageNum){
				this.$API.POST('/detail/currencyLogs/logs', {currentPage:currentPage, pageNum:pageNum, filter:this.filterData}).then(({data})=>{
					this.logs =  data.data ;
			...
			getOptionData(){
				this.$API.POST('/detail/currencyLogs/options').then(({data})=>{
